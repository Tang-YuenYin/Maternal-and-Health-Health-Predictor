...
# # Feature and target Introduction 
...
# # Define and declare all regression model
# 
# 
...
# The list of scores for each fold is [0.64978903, 0.61016949, 0.63135593]. These scores represent the performance of a model on different subsets of the data during the cross-validation process.
# 
# The average score, calculated as the mean of all the fold scores, is 0.6304381510882262. This average score provides an overall indication of the model's performance across all the folds in the cross-validation process.
...
# The list of scores for each fold is [0.65822785, 0.65677966, 0.68644068]. These scores represent the performance of a model on different subsets of the data during the cross-validation process.
# 
# The average score, calculated as the mean of all the fold scores, is 0.6671493956947723. This average score provides an overall indication of the model's performance across all the folds in the cross-validation process.
...
# 
# 
# The list of scores for each fold is as follows: [0.87323944, 0.88732394, 0.85915493, 0.88732394, 0.88732394, 0.81690141, 0.83098592, 0.94366197, 0.78873239, 0.88571429]. These scores represent the performance of a model on different subsets of the data during the cross-validation process.
# 
# The average score, calculated as the mean of all the fold scores, is 0.8660362173038229. This average score provides an overall indication of the model's performance across all the folds in the cross-validation process.
...
# 
# The list includes the scores for each fold: [0.70422535, 0.71830986, 0.70422535, 0.71830986, 0.6056338, 0.71830986, 0.64788732, 0.73239437, 0.64788732, 0.64285714]. These scores represent the performance of a model on different subsets of the data during the cross-validation process.
# 
# The average score, calculated as the mean of all the fold scores, is 0.684004024144869. This average score gives an overall indication of the model's performance across all the folds in the cross-validation process.
...
for estimator in estimators:
    ...
    scores.append([estimator, avg_score])

table_headers = ["Estimators", "Average Score"]
table = tabulate(scores, headers=table_headers, tablefmt="grid")
print(table)
...
for name, clf in classifiers:
    cv_scores = cross_val_score(clf, X_res, y_res, cv=10)
    avg_score = np.average(cv_scores)
    scores.append([name, cv_scores, avg_score])

table_headers = ["Classifier", "Cross-Validation Scores", "Average Score"]
table = tabulate(scores, headers=table_headers, tablefmt="grid")
print(table)
...
# The overall accuracy of the model is 0.81, indicating that it correctly predicted 81% of all instances across all classes.
# 
# In summary, the model shows reasonably good performance across all classes, with similar precision, recall, and F1-scores. 
...
# Defining a parameter grid for selecting the best parameters for the XGBoost model
param_grid = {
...
# Applying param_grid , k_fold as 3 
...
# Selecting the column names for the feature variables and the unique risk levels
feature_names = data.columns[0:6]
class_names = data["RiskLevel"].unique().tolist()

# Creating a Decision Tree classifier with entropy as the criterion and a maximum depth of 4
classifier = DecisionTreeClassifier(criterion="entropy", max_depth=4)
classifier.fit(X, Y)

# Creating a plot to visualize the Decision Tree
